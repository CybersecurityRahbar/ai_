...
/** Independent Advanced search service. It consumes the shared corpus but keeps its own evidence and ranking. */
        ...
        try {
            ...
            for ((index, entity) in stored.withIndex()) {
                ...
                if (index % 16 == 0 || index == stored.lastIndex) {
                    onProgress(index + 1, stored.size, "تحليل Advanced V2 • ${index + 1}/${stored.size}")
                }
            }

            val candidates = scored.sortedByDescending { it.second.similarity }.take(50)
            val items = withContext(Dispatchers.IO) {
                itemDao.getByIds(candidates.map { it.first.itemId }).associateBy { it.id }
            }

            return@withContext candidates.mapNotNull { (entity, score) ->
                ...
                val globalAgreement = listOf(
                    score.structure,
                    score.spatialColor,
                    score.spatialTexture,
                    score.gradient,
                    score.illumination
                ).count { it >= 0.62f }

                // Advanced is not allowed to turn a weak structural match into a high-confidence result.
                var final = if (base != null) {
                    baseScore * 0.58f + score.similarity * 0.42f
                } else {
                    score.similarity * 0.70f
                }
                if (globalAgreement <= 1 && final > 0.55f) final *= 0.78f
                if (base != null && base.ransacInliers == 0 && score.structure < 0.50f) final *= 0.90f
                if (globalAgreement >= 3 && score.gradient >= 0.65f && score.illumination >= 0.60f) final = (final + 0.025f).coerceAtMost(1f)
                ...
                val reasons = buildList {
                    ...
                    if (globalAgreement >= 3) add("independent_signal_consensus")
                    if (globalAgreement <= 1) add("weak_cross_signal_consensus")
                ...
                Evidence(
                    ...
                    gradientMagnitudePercent = ((score.gradient * 0.68f + score.gradient * 0.32f) * 100f).toInt().coerceIn(0, 100),
