...
class ReverseImageSearchService(context: Context) : AutoCloseable {
    ...
    suspend fun buildIndex(rebuild: Boolean = false, onProgress: (IndexProgress) -> Unit = {}): IndexProgress = withContext(Dispatchers.Default) {
        ...
        var indexed = 0; var skipped = 0; var failed = 0
        for ((index, item) in items.withIndex()) {
            try {
                val existing = withContext(Dispatchers.IO) { fingerprintDao.getForItem(item.id) }
                val unchanged = existing != null && existing.engineVersion == HaarFingerprintEngine.ENGINE_VERSION && existing.sourceModifiedAt == item.sourceModifiedAt
                ...
                failed++; run.failure("ITEM_${item.id}", t)
            } finally {
                onProgress(IndexProgress(index + 1, items.size, indexed, skipped, failed))
            }
        }
        run.success("Reverse-image fingerprint index completed", mapOf("items" to items.size.toString(), "indexed" to indexed.toString(), "skipped" to skipped.toString(), "failed" to failed.toString()))
        IndexProgress(items.size, items.size, indexed, skipped, failed)
    }

    suspend fun search(queryUri: Uri, limit: Int = 50, minimumSimilarity: Float = 0f): List<Result> = withContext(Dispatchers.Default) {
        ...
        if (fingerprints.isEmpty()) throw IllegalStateException("لا توجد بصمات في فهرس البحث العكسي. أضف الصور ثم ابنِ الفهرس.")
        val items = withContext(Dispatchers.IO) { itemDao.getByIds(fingerprints.map { it.itemId }.distinct()).associateBy { it.id } }
        val results = fingerprints.mapNotNull { fp ->
            if (fp.engineVersion != HaarFingerprintEngine.ENGINE_VERSION) return@mapNotNull null
            val item = items[fp.itemId] ?: return@mapNotNull null
            val score = engine.compare(query, HaarFingerprintEngine.Fingerprint(fp.width, fp.height, fp.channels, fp.signature))
            if (score.similarity < minimumSimilarity) null else Result(item, score.similarity, (score.similarity * 100f).toInt().coerceIn(0, 100), score.matchedCoefficients)
        }.sortedByDescending { it.similarity }.take(limit)
        run.success("Reverse-image search completed", mapOf("indexed" to fingerprints.size.toString(), "results" to results.size.toString()))
        results
    }

    private fun decodeAndFingerprint(uri: Uri): HaarFingerprintEngine.Fingerprint {
        val bitmap = resolver.openInputStream(uri).use { input -> requireNotNull(BitmapFactory.decodeStream(input)) { "تعذر قراءة الصورة: $uri" } }
        return try { engine.fingerprint(bitmap) } finally { bitmap.recycle() }
    }
