...
class DataCenterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(
            action("EXPORT KNOWLEDGE BASE", "Create a complete local .pmai backup") {
                exportPicker.launch("PersonalMemory_Backup_${System.currentTimeMillis()}.pmai")
            },
            margin()
        )
        root.addView(
            action("IMPORT KNOWLEDGE BASE", "Restore local evidence and indexes") {
        ...
        root.addView(
            action("IMPORT MOBILECLIP", "Install + validate local TFLite model") {
                modelPicker.launch(arrayOf("application/octet-stream", "application/tflite", "*/*"))
            },
            margin()
        )
        root.addView(
            action("REMOVE MOBILECLIP", "Delete local visual model") {
                modelManager.deleteModel()
                updateStats()
                status.text = "MOBILECLIP REMOVED"
                status.setTextColor(red)
            },
            margin()
        )
        root.addView(
            action("IMPORT FACENET-512", "Install + validate 160×160 / 512-D model") {
                faceModelPicker.launch(arrayOf("application/octet-stream", "application/tflite", "*/*"))
            },
            margin()
        )
        root.addView(
            action("REMOVE FACENET-512", "Delete local identity model") {
                faceNetManager.deleteModel()
                updateStats()
                status.text = "FACENET-512 REMOVED"
                status.setTextColor(red)
            },
            margin()
        )
    ...
    private fun header() = LinearLayout(this).apply {
        ...
        addView(TextView(this@DataCenterActivity).apply {
            text = "◈ INTELLIGENCE COMMAND / DATA CENTER"
            textSize = 10f
            setTextColor(cyan)
        ...
        addView(TextView(this@DataCenterActivity).apply {
            text = "DATA & MODEL CENTER"
            textSize = 27f
            setTextColor(this@DataCenterActivity.text)
            setTypeface(null, Typeface.BOLD)
            setPadding(0, dp(3), 0, dp(2))
        })

        addView(TextView(this@DataCenterActivity).apply {
            text = "EVIDENCE • INDEX • BACKUP • MODEL INTEGRITY"
            textSize = 9f
            setTextColor(muted)
        })
    }
        ...
        lifecycleScope.launch {
            val snapshot = withContext(Dispatchers.IO) {
                ...
                Stats(
                    ...
                    persons = db.personDao().count(),
                    embeddings = db.embeddingDao().count()
                )
            }

            val databaseSize = formatBytes(
                getDatabasePath("personal_memory.db").length()
            )
    ...
    private fun setBusy(busy: Boolean, message: String) {
        status.text = message
        status.setTextColor(if (busy) cyan else neon)
    }
    ...
    private fun exportBackup(uri: android.net.Uri) {
        lifecycleScope.launch {
            ...
            try {
                val result = backupManager.exportBackup(uri) { progress ->
                    runOnUiThread { status.text = "◉ EXPORT ${progress}%" }
                }
                updateStats()
                if (result.missingImageIds.isEmpty()) {
                    run.success("Backup export completed")
                } else {
                    run.warning(
                        "Backup exported with missing images",
                        mapOf("missing" to result.missingImageIds.size.toString())
                    )
                }
                status.text = "● EXPORT COMPLETE • ${result.copiedImages}/${result.imageCount} IMAGES"
                status.setTextColor(
                    if (result.missingImageIds.isEmpty()) neon else Color.rgb(255, 193, 72)
                )
            } catch (error: Exception) {
                run.failure("EXPORT", error)
                status.text = "● EXPORT ERROR • ${error.message}"
                status.setTextColor(red)
            }
        }
    }
    ...
    private fun importBackup(uri: android.net.Uri) {
        lifecycleScope.launch {
            ...
            try {
                val result = backupManager.importBackup(uri) { progress ->
                    runOnUiThread { status.text = "◉ RESTORE ${progress}%" }
                }
                updateStats()
                if (result.restoredImages == result.imageCount) {
                    run.success("Backup import completed")
                } else {
                    run.warning("Backup imported with missing images")
                }
                status.text = "● RESTORE COMPLETE • ${result.restoredImages}/${result.imageCount} IMAGES"
                status.setTextColor(
                    if (result.restoredImages == result.imageCount) neon else Color.rgb(255, 193, 72)
                )
            } catch (error: Exception) {
                run.failure("IMPORT", error)
                status.text = "● RESTORE ERROR • ${error.message}"
                status.setTextColor(red)
            }
        }
    }
            ...
            try {
                withContext(Dispatchers.IO) {
                    modelManager.importModel(uri) { copied, total ->
                        runOnUiThread {
                            status.text = "◉ MOBILECLIP ${if (total > 0) ((copied * 100L) / total).toInt() else 0}%"
                        }
                    }
                }
                updateStats()
                run.success("MobileCLIP imported and runtime-validated")
                status.text = "● MOBILECLIP READY • TFLITE + RUNTIME VALIDATED"
                status.setTextColor(neon)
            } catch (error: Exception) {
                run.failure("IMPORT", error)
                status.text = "● MOBILECLIP ERROR • ${error.message}"
                status.setTextColor(red)
            }
        }
    }
            ...
            try {
                withContext(Dispatchers.IO) {
                    faceNetManager.importModel(uri) { copied, total ->
                        runOnUiThread {
                            status.text = "◉ FACENET-512 ${if (total > 0) ((copied * 100L) / total).toInt() else 0}%"
                        }
                    }
                }
                updateStats()
                run.success("FaceNet-512 imported and inference-validated")
                status.text = "● FACENET-512 READY • 512-D / RUNTIME VALIDATED"
                status.setTextColor(neon)
            } catch (error: Exception) {
                run.failure("IMPORT", error)
                status.text = "● FACENET-512 ERROR • ${error.message}"
                status.setTextColor(red)
            }
        }
    }
