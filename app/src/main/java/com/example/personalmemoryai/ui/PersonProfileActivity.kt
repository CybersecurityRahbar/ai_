...
class PersonProfileActivity : AppCompatActivity() {
    ...
    private fun header(person: com.example.personalmemoryai.database.PersonEntity) = LinearLayout(this).apply {
        ...
        addView(TextView(this@PersonProfileActivity).apply { text = person.displayName?.takeIf { it.isNotBlank() } ?: "UNKNOWN SUBJECT"; textSize = 27f; setTextColor(this@PersonProfileActivity.primaryText); setTypeface(null, Typeface.BOLD); setPadding(0, dp(4), 0, dp(2)) })
    ...
    private fun message(title: String, body: String, critical: Boolean) = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(14), dp(14), dp(14), dp(14)); setBackgroundResource(if (critical) com.example.personalmemoryai.R.drawable.bg_critical_alert else com.example.personalmemoryai.R.drawable.bg_neon_panel); addView(TextView(this@PersonProfileActivity).apply { text = title; textSize = 13f; setTextColor(if (critical) Color.rgb(255,48,79) else this@PersonProfileActivity.primaryText); setTypeface(null, Typeface.BOLD) }); addView(TextView(this@PersonProfileActivity).apply { text = body; textSize = 10f; setTextColor(muted); setPadding(0, dp(5), 0, 0) }) }
