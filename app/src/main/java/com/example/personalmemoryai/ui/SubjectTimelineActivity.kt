 ...
 *
 * Presentation-only: consumes indexed Room evidence and never reruns OCR, face analysis,
 * object detection, semantic inference, or the future TextEncoder.
 */
class SubjectTimelineActivity : AppCompatActivity() {
    ...
    private fun header(name: String, id: Long, images: Int, faces: Int) = LinearLayout(this).apply {
        ...
        addView(TextView(this@SubjectTimelineActivity).apply { text = name.ifBlank { "UNKNOWN SUBJECT" }; textSize = 26f; setTextColor(text); setTypeface(null, Typeface.BOLD); setPadding(0, dp(4), 0, dp(2)) })
    ...
    private fun eventCard(sequence: Int, image: ImageEntity, faceCount: Int): View {
        val box = LinearLayout(this).apply {
            ...
            isClickable = true
            setOnClickListener { ImageViewerActivity.start(this@SubjectTimelineActivity, image.uri) }
        }
        ...
        info.addView(TextView(this).apply { text = image.fileName; textSize = 13f; setTextColor(text); setTypeface(null, Typeface.BOLD); setPadding(0, dp(5), 0, dp(4)); maxLines = 2 })
        ...
        info.addView(TextView(this).apply { text = "INDEXED  ${formatTime(image.indexedAt)}  •  ${image.width}×${image.height}"; textSize = 7.5f; setTextColor(muted); setPadding(0, dp(5), 0, 0) })
        box.addView(info)
        return box
    }
