...
class ImageIntelligenceActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            val data = withContext(Dispatchers.IO) {
                ...
                val ocr = db.imageDao().countWithOcr()
                val objects = db.imageDao().countWithDetectedObjects()
                Quad(images, imageEmbeddings, ocr, objects)
            }
            ...
            list.adapter = EvidenceAdapter(data.images) { image -> ImageViewerActivity.start(this@ImageIntelligenceActivity, image.uri) }
        }
    }
    ...
    private fun header() = LinearLayout(this).apply {
        ...
        addView(TextView(this@ImageIntelligenceActivity).apply { text = "IMAGE INTELLIGENCE"; textSize = 27f; setTextColor(text); setTypeface(null, Typeface.BOLD); setPadding(0, dp(3), 0, dp(2)) })
    ...
    private fun telemetry(data: Quad) = LinearLayout(this).apply {
        ...
        listOf(
            "IMAGES" to data.images.size.toString(),
            "VISUAL EMB" to data.imageEmbeddings.toString(),
            "OCR" to data.ocr.toString(),
            "OBJECTS" to data.objects.toString()
    ...
    private class EvidenceAdapter(
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
            val box = LinearLayout(parent.context).apply {
                orientation = LinearLayout.VERTICAL
                setPadding(dp(parent.context, 10), dp(parent.context, 10), dp(parent.context, 10), dp(parent.context, 10))
                setBackgroundResource(com.example.personalmemoryai.R.drawable.bg_neon_panel)
                layoutParams = RecyclerView.LayoutParams(-1, -2).apply { bottomMargin = dp(parent.context, 9) }
            }
            ...
            box.addView(image); box.addView(title); box.addView(details)
            return Holder(box, image, title, details)
        }
        override fun onBindViewHolder(holder: Holder, position: Int) {
            val image = items[position]
            holder.image.setImageURI(android.net.Uri.parse(image.uri))
            holder.title.text = "${position + 1}. ${image.fileName}"
            holder.details.text = "OCR  ${if (image.ocrText.isBlank()) "NONE" else "AVAILABLE"}  •  QUALITY  ${String.format(Locale.US, "%.2f", image.ocrQualityScore)}\nOBJECTS  ${if (image.detectedObjects.isBlank() || image.detectedObjects == "[]") "NONE" else "INDEXED"}\n${image.width}×${image.height}  •  ${image.mimeType ?: "IMAGE"}\nPATH  ${image.filePath ?: image.uri}"
            holder.itemView.setOnClickListener { click(image) }
        }
