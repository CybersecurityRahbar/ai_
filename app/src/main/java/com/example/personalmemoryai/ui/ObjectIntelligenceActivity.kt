...
class ObjectIntelligenceActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(health, margin())
        root.addView(action("RUN OBJECT MODEL TEST", "Select an image and execute the real YOLO pipeline") { picker.launch("image/*") }, margin())
        root.addView(action("SEARCH OBJECT EVIDENCE", "Search persisted object observations and source images") { showObjectSearch() }, margin())
        root.addView(section("PERSISTED OBJECT EVIDENCE"), margin())
        results = TextView(this).apply { textSize = 11f; setTextColor(text); setPadding(dp(14), dp(14), dp(14), dp(14)); setBackgroundResource(com.example.personalmemoryai.R.drawable.bg_neon_panel); setTextIsSelectable(true) }
    ...
    private fun header() = LinearLayout(this).apply {
        ...
        addView(TextView(this@ObjectIntelligenceActivity).apply { text = "OBJECT INTELLIGENCE"; textSize = 27f; setTextColor(text); setTypeface(null, Typeface.BOLD); setPadding(0, dp(3), 0, dp(2)) })
    ...
    private fun action(title: String, subtitle: String, click: () -> Unit) = Button(this).apply {
        text = "▸  $title\n    $subtitle"; textSize = 10f; gravity = Gravity.START or Gravity.CENTER_VERTICAL; setTextColor(text); setAllCaps(false); setBackgroundResource(com.example.personalmemoryai.R.drawable.bg_neon_action); setPadding(dp(16), dp(8), dp(16), dp(8)); setOnClickListener { click() }
    ...
    private fun refresh() {
        lifecycleScope.launch {
            ...
            health.setTextColor(neon)
            results.text = if (snapshot.third.isEmpty()) "NO PERSISTED OBJECT OBSERVATIONS\n\nRun image indexing or the model test above." else snapshot.third.joinToString("\n") { "${it.label.padEnd(22, ' ')}  ${it.total} observations" }
        }
    }

    private fun showObjectSearch() {
        val input = EditText(this).apply { hint = "person / car / phone / dog …"; setTextColor(text); setHintTextColor(muted); setSingleLine(true) }
