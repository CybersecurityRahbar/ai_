...
class FaceSearchActivity : AppCompatActivity() {
    ...
    private fun addCard(rank: Int, result: FaceSearchService.FaceMatch) {
        ...
        val scoreBar = View(this).apply {
            setBackgroundColor(scoreColor)
            alpha = 0.85f
            layoutParams = LinearLayout.LayoutParams(dp((result.compositeScore.coerceIn(0f, 1f) * 100f).toInt().coerceAtLeast(2)), dp(3)).apply { bottomMargin = dp(7) }
        }
        card.addView(scoreBar)
        card.addView(TextView(this).apply {
            text = String.format(Locale.US, "IDENTITY %.1f%%   •   SHAPE %.1f%%\nPOSE %.1f%%   •   QUALITY %.1f%%\nCONFIDENCE BAND  %s", result.identitySimilarity * 100f, result.shapeSimilarity * 100f, result.poseSimilarity * 100f, result.quality * 100f, result.confidenceBand.name)
            textSize = 10f
            setTextColor(cyan)
        ...
        card.animate().alpha(1f).setDuration(240L).start()
    }
    ...
    private fun addMessage(title: String, message: String, critical: Boolean) {
        ...
        resultHost.addView(card)
        if (critical) {
            ValueAnimator.ofFloat(0.55f, 1f, 0.55f).apply {
                duration = 900L
                repeatCount = 2
                addUpdateListener { card.alpha = it.animatedValue as Float }
                start()
            }
        }
    }
