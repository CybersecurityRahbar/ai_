...
class DataBackupManager(private val context: Context) {
    ...
    suspend fun exportBackup(destination: Uri, progress: (Int) -> Unit = {}): ExportResult = withContext(Dispatchers.IO) {
        ...
        try {
            run.stage("LOAD_DATABASE", "Loading indexed image records")
            ...
            try {
                resolver.openOutputStream(destination, "w")?.use { raw -> ZipOutputStream(raw.buffered()).use { zip ->
                    zip.putNextEntry(ZipEntry(DB_ENTRY)); FileInputStream(dbFile).use { it.copyTo(zip) }; zip.closeEntry()
                    val manifest = buildString { appendLine("format=PersonalMemoryAI-2"); appendLine("database=personal_memory.db"); appendLine("imageCount=${images.size}"); appendLine("createdAt=${System.currentTimeMillis()}") }
                    zip.putNextEntry(ZipEntry(MANIFEST_ENTRY)); zip.write(manifest.toByteArray(Charsets.UTF_8)); zip.closeEntry()
                    images.forEachIndexed { index, image ->
                        val entryName = "$IMAGE_PREFIX${image.id}_${safeName(image.fileName)}"
                        openUri(image.uri)?.use { input -> zip.putNextEntry(ZipEntry(entryName)); input.copyTo(zip); zip.closeEntry(); copied++ } ?: missing.add(image.id)
                        progress(((index + 1) * 100 / images.size.coerceAtLeast(1)))
                    }
                } } ?: error("تعذر فتح ملف الوجهة للكتابة")
            } finally { AppDatabase.getInstance(context) }
    ...
    suspend fun importBackup(source: Uri, progress: (Int) -> Unit = {}): ImportResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            ZipFile(archive).use { zip ->
                ...
                File(currentDb.path + "-wal").delete(); File(currentDb.path + "-shm").delete()
                ...
                imageEntries.forEachIndexed { index, entry ->
                    ...
                    restored++; progress(((index + 1) * 100 / imageEntries.size.coerceAtLeast(1)))
                }
