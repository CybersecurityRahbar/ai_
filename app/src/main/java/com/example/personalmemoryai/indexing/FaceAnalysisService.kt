...
class FaceAnalysisService(
    ...
    suspend fun analyze(bitmap: Bitmap): List<AnalyzedFace> {
        ...
        for (detection in detections) {
            ...
            try {
                // Quality is recorded as metadata, not used as a hard gate.
                // A clear face can have low brightness/contrast and must still
                // receive an embedding so clustering/search remains complete.
                val quality = qualityAnalyzer.analyze(crop)
                val embeddingResult = try { embeddingModel.generateEmbedding(crop) } catch (_: Throwable) { null }
                val normalized = embeddingResult?.let(FaceSimilarity::normalize)
                val valid = normalized != null && normalized.isNotEmpty() && normalized.all { it.isFinite() }
    ...
    private fun emptyResult(detection: FaceLandmarkResult, quality: FaceQualityAnalyzer.QualityResult?) =
        AnalyzedFace(
            detection = detection,
