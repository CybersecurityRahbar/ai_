...
public data class ArabicOcrResult(
    val text: String,
...
class ArabicOcrEngine(private val context: Context) {
    ...
    fun recognizeDetailed(uri: Uri): ArabicOcrResult {
        ...
        val options = android.graphics.BitmapFactory.Options().apply { inSampleSize = sample; inPreferredConfig = Bitmap.Config.ARGB_8888; inScaled = false }
        val bitmap = context.contentResolver.openInputStream(uri)?.use { android.graphics.BitmapFactory.decodeStream(it, null, options) } ?: return ArabicOcrResult("", 0f, 0, 0)
    ...
    fun recognizeDetailed(bitmap: Bitmap): ArabicOcrResult {
        ...
        return try {
            ...
            for (working in variants) {
                for (mode in intArrayOf(TessBaseAPI.PageSegMode.PSM_AUTO, TessBaseAPI.PageSegMode.PSM_SINGLE_BLOCK, TessBaseAPI.PageSegMode.PSM_SPARSE_TEXT, TessBaseAPI.PageSegMode.PSM_SINGLE_LINE)) {
                    passes++
                    try {
                        ...
                        if (text.isNotBlank()) { candidates += Candidate(text, confidence, mode); successful++ }
                        engine.clear()
                    } catch (_: Throwable) {}
                }
            ...
            val quality = if (merged.isBlank()) 0f else ((bestConfidence / 100f) * 0.55f + consistency * 0.25f + textDensity * 0.20f).coerceIn(0f, 1f)
    ...
    private fun prepareVariants(source: Bitmap): List<Bitmap> {
        ...
        if (maxOf(width, height) < 2800) {
            ...
            variants += Bitmap.createScaledBitmap(base, (width * scale).toInt(), (height * scale).toInt(), true)
        }
        val gray = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        Canvas(gray).drawBitmap(base, 0f, 0f, Paint(Paint.ANTI_ALIAS_FLAG).apply { colorFilter = ColorMatrixColorFilter(ColorMatrix().apply { setSaturation(0f) }) })
        variants += gray
        val contrast = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val contrastMatrix = ColorMatrix(floatArrayOf(1.35f, 0f, 0f, 0f, -35f, 0f, 1.35f, 0f, 0f, -35f, 0f, 0f, 1.35f, 0f, -35f, 0f, 0f, 0f, 1f, 0f))
        Canvas(contrast).drawBitmap(base, 0f, 0f, Paint(Paint.ANTI_ALIAS_FLAG).apply { colorFilter = ColorMatrixColorFilter(contrastMatrix) })
        variants += contrast
    ...
    private fun sanitizeCandidate(value: String): String = value.lineSequence().map { it.replace(Regex("[\\u0000-\\u001F]"), " ").trim() }.filter { line -> line.any { it.isLetterOrDigit() || it in "،؛,:.!؟?-/()[]%+=" } }.joinToString("\n").replace(Regex("[ \\t]+"), " ").trim()
    private fun selectAndMergeCandidates(candidates: List<Candidate>): String {
        if (candidates.isEmpty()) return ""
        val ranked = candidates.distinctBy { normalizeForComparison(it.text) }.sortedByDescending { candidateValue(it) }
        val pool = ranked.filter { it.confidence >= 35f || it.text.length >= 12 }.ifEmpty { ranked }
        val lines = linkedMapOf<String, String>()
        for (candidate in pool.take(6)) for (line in candidate.text.lineSequence()) {
            val clean = line.trim(); if (clean.length < 2) continue
            val key = normalizeForComparison(clean); if (key.isBlank()) continue
            if (lines.keys.firstOrNull { similarText(it, key) } == null) lines[key] = clean
        }
        return lines.values.joinToString("\n")
    }
    private fun candidateValue(candidate: Candidate): Float = candidate.confidence + (candidate.text.length / 500f).coerceIn(0f, 1f) * 20f + if (candidate.mode == TessBaseAPI.PageSegMode.PSM_AUTO) 3f else 0f
    private fun candidateConsistency(candidates: List<Candidate>, merged: String): Float { if (candidates.isEmpty() || merged.isBlank()) return 0f; val target = normalizeForComparison(merged); val supporting = candidates.count { val value = normalizeForComparison(it.text); value.length >= 4 && (value.contains(target.take(minOf(20, target.length))) || target.contains(value.take(minOf(20, value.length)))) }; return (supporting.toFloat() / candidates.size).coerceIn(0f, 1f) }
    private fun textDensity(text: String): Float { if (text.isBlank()) return 0f; return (text.count { it.isLetterOrDigit() }.toFloat() / text.length.coerceAtLeast(1)).coerceIn(0f, 1f) }
    private fun normalizeForComparison(value: String): String = value.replace(Regex("[\\u064B-\\u065F\\u0670]"), "").replace(Regex("[\\u0622\\u0623\\u0625]"), "ا").replace('ى', 'ي').replace('ة', 'ه').replace('ـ', ' ').replace(Regex("\\s+"), "").lowercase()
    private fun similarText(a: String, b: String): Boolean { if (a == b) return true; val short = minOf(a.length, b.length); if (short < 6) return false; val prefix = minOf(12, short); return a.take(prefix) == b.take(prefix) || a.takeLast(prefix) == b.takeLast(prefix) }
    fun close() { try { tess?.clear() } catch (_: Throwable) {}; try { tess?.recycle() } catch (_: Throwable) {}; tess = null }
}
