...
class FileTFLiteFaceEmbeddingModel(
    ...
    override val modelVersion: String = "file-tflite"
    ...
    init {
        ...
        require(shape.size == 4 && shape[0] == 1 && shape[3] == 3) {
            "Face model input must be [1,H,W,3]: ${shape.contentToString()}"
        }
        inputHeight = shape[1]
        inputWidth = shape[2]
    ...
    override suspend fun generateEmbedding(faceBitmap: Bitmap): FloatArray {
        ...
        for (pixel in pixels) {
            put(input, (pixel shr 16) and 0xFF)
            put(input, (pixel shr 8) and 0xFF)
            put(input, pixel and 0xFF)
        }
        input.rewind()
        val output = Array(1) { FloatArray(outputDimension) }
        interpreter.run(input, output)
        if (resized !== faceBitmap && !resized.isRecycled) resized.recycle()
        return FaceSimilarity.normalize(output[0])
    }
