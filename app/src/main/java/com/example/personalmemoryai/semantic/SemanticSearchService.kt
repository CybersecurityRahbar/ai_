...
class SemanticSearchService(context: Context) : AutoCloseable {
    ...
    suspend fun ensureModel() {
        ...
        try { check(encoder.load()) { "تعذر تحميل نموذج MobileCLIP-S2" } }
        catch (t: Throwable) { diagnostics.record("VISUAL_ENGINE", "MODEL_LOAD", DiagnosticsManager.Severity.ERROR, "Failed to load MobileCLIP-S2", t); throw t }
    }

    suspend fun importModel(source: Uri, onProgress: (Long, Long) -> Unit = { _, _ -> }) {
        modelManager.importModel(source, onProgress); encoder.close()
    }
    ...
    suspend fun indexAllImages(onProgress: (processed: Int, total: Int, embedded: Int, skipped: Int) -> Unit = { _, _, _, _ -> }) = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            for (image in images) {
                try {
                    ...
                    if (existing != null) skipped++ else { indexImageAndStore(image); embedded++ }
                } catch (t: Throwable) {
                    failed++
                    run.failure("IMAGE_${image.id}", t)
                } finally {
                    processed++; onProgress(processed, total, embedded, skipped)
                }
            }
            if (total > 0 && embedded + skipped == 0) run.warning("No visual embeddings were produced or reused", mapOf("total" to total.toString()))
            run.success("Visual index completed", mapOf("processed" to processed.toString(), "embedded" to embedded.toString(), "skipped" to skipped.toString(), "failed" to failed.toString()))
    ...
    suspend fun searchSimilarImages(queryUri: Uri, limit: Int = 30): List<ScoredImage> = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            for (embedding in embeddings) {
                if (embedding.modelName != MobileClipImageEncoder.MODEL_NAME || embedding.modelVersion != MobileClipImageEncoder.MODEL_VERSION || embedding.dimension != query.size) continue
                compatible++
            ...
            if (embeddings.isNotEmpty() && compatible == 0) run.warning("No embeddings compatible with active model/version", mapOf("loaded" to embeddings.size.toString()))
    ...
    private fun cosine(a: FloatArray, b: FloatArray): Float {
        var dot = 0.0; var normA = 0.0; var normB = 0.0; val size = minOf(a.size, b.size)
        for (i in 0 until size) { dot += a[i].toDouble() * b[i].toDouble(); normA += a[i].toDouble() * a[i].toDouble(); normB += b[i].toDouble() * b[i].toDouble() }
