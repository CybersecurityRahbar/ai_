...
/** Local visual semantic search. TextEncoder is intentionally deferred. */
class SemanticSearchService(context: Context) : AutoCloseable {
    ...
    suspend fun indexAllImages(onProgress: (processed: Int, total: Int, embedded: Int, skipped: Int, failed: Int) -> Unit = { _, _, _, _, _ -> }) = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            for (image in images) {
                ...
                } catch (t: Throwable) { failed++; run.failure("IMAGE_${image.id}", t) }
    ...
    private fun cosine(a: FloatArray, b: FloatArray): Float {
        var dot = 0.0; var normA = 0.0; var normB = 0.0; require(a.size == b.size) { "Embedding dimensions differ: ${a.size} vs ${b.size}" }
